    ...
    mission_(IDLE)
{
    ...
    uav_state_sub_ = nh.subscribe<mavros_msgs::State>("mavros/state", 10, &controller::uav_state_cb, this);
    ...
    vision_timer_ = nh.createTimer(ros::Duration(0.05), &controller::vision_timer_cb, this);
...
void controller::mainloop_timer_cb(const ros::TimerEvent &event){
    switch(mission_){
        ...
        case LANDING:
            ...
            // change to IDLE
            if (uav_state_.connected && !uav_state_.armed && uav_state_.mode != "OFFBOARD"){
                mission_ = IDLE;
            }
